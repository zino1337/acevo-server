#!/usr/bin/env bash
# Runs the AC EVO dedicated server once, in the foreground (its lifetime = the server's).
# Invoked by the dashboard (dashboard/server_control.py) on Start / Restart, NOT by the
# container entrypoint — so a Steam/server failure here never takes the dashboard down.
...
[[ -x "${PROTON_BIN}" ]] || {
  echo "ERROR: Proton launcher missing: ${PROTON_BIN}" >&2
  exit 1
}
[[ -x "${XVFB_BIN}" ]] || {
  echo "ERROR: Xvfb missing: ${XVFB_BIN}" >&2
  exit 1
}
[[ -f "${PAYLOAD_GENERATOR}" ]] || {
  echo "ERROR: Launch payload generator missing: ${PAYLOAD_GENERATOR}" >&2
  exit 1
}
...
[[ -n "${SERVER_PAYLOAD}" ]] || {
  echo "ERROR: Empty server payload." >&2
  exit 1
}
[[ -n "${SEASON_PAYLOAD}" ]] || {
  echo "ERROR: Empty season payload." >&2
  exit 1
}
